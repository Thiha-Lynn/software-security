...
**AI-tool usage disclosure (required):** Claude (Anthropic) — used to set up the lab environment
(fork/clone, Docker, proxies), to draft this worksheet, implement the Task 8 fix, and as the
subject of the *Audit the AI* task. Findings were verified against the running app on `:8081`.
...
`threat-model-6631503092.md §9`. Fix commit **`2202101`** on `wk01`.
...
**3. Correct, verified version** (what I committed, `2202101`):
```python
...
*Verified:* applying it produced commit `2202101`; re-running `POST /upload filename=../pwned.txt`
